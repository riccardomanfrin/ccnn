...
class Neuron {
   public:
    ...
    Neuron(int num_inputs) : num_inputs(num_inputs) {
        if (num_inputs > 0) {
            weights = (value_t *)malloc(sizeof(value_t) * num_inputs);
        }
    }
    ~Neuron() { free(weights); }
};
...
class CNN {
   ...
   private:
    ...
    int calc_layer(int prev_layer_inputs, Neuron **prev_layer, int layer_inputs,
                   Neuron **layer);
    int calc_output_error(int layer_neurons, Neuron **layer, Neuron **expected_values);
    int normalize_output_error(int layer_neurons, Neuron **layer);
    int back_propagate_errors(int layer_neurons, Neuron **layer,
                              int prev_layer_neurons, Neuron **prev_layer);
    int adjust_weights_and_biases(int layer_neurons, Neuron **layer,
                                  int prev_layer_neurons, Neuron **prev_layer,
                                  int batches_size);
};
